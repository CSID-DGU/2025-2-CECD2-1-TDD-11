...
@Tag(name = "회원 V2 (Member V2)", description = "회원 관련 API V2")
@Logging
public class MemberControllerV2 {

	private final MemberFacadeServiceV2 memberFacadeServiceV2;

	@Operation(summary = "회원 정보 조회", description = "회원 메타데이터를 조회합니다.")
	@ApiResponses(value = {
	...
	@GetMapping("/me")
	public MemberBasicResponseDtoV2 getMember(
			@LoginMemberInfo MemberSessionDto memberSessionDto
	) {
		return memberFacadeServiceV2.getMember(memberSessionDto.getMemberId());
	}

	@Operation(summary = "회원 정보 수정 요청", description = "회원 메타데이터에 대한 수정을 요청합니다.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "ok"),
	})
	@ApiErrorCodeExample(
	...
	public void updateMember(
			@LoginMemberInfo MemberSessionDto memberSessionDto,
			@Valid @ModelAttribute MemberUpdateRequestDtoV2 requestDto
	) {
		memberFacadeServiceV2.updateMember(memberSessionDto.getMemberId(), requestDto);
	}
}
